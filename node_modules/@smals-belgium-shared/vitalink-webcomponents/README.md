...
| security-label     | The security label to append to the FHIR queries (Must be provided when calling Vitalink server)                                                                                                                                        | false                                                                                                                            | Any security label as string                 |
...
```html
<vitalink-audit-trail-table server="http://localhost:3031" auth-token="ey1531eaz2131eaz1a1e32za1e5za1e35za1ez53a1" patient-identifier="123" patient-ssin="00000000000" security-label="https://www.apps.vitalink-services.be|VITALINK"></vitalink-audit-trail-table>
```
...
wc.setAttribute("auth-token", config.accessToken);
wc.setAttribute("security-label", config.securityLabel);
document.body.appendChild(wc);
```
...
- fixup typo in resource name mapping and use urls as fallback

### 0.0.10

- fixup 'NO DATA' for core DZOP resources names

### 0.0.9

- Improve resource type translation mapping (now based on security label)
- Display a specific error message on 403 server error
- Fixup translations

### 0.0.8

- Update resource names mapping
- Display first name and last name
- Display a message if too much results are found or on server error
...
```html
<vitalink-care-plan-table server="http://localhost:3031" auth-token="ey1531eaz2131eaz1a1e32za1e5za1e35za1ez53a1" patient-identifier="123" language="nl" security-label="https://www.apps.vitalink-services.be|PUBLIC_HEALTH"></vitalink-care-plan-table>
```
...
- Limit Batch calls to 10 entries
...
```html
<vitalink-vaccination-table server="http://localhost:3031" auth-token="ey1531eaz2131eaz1a1e32za1e5za1e35za1ez53a1" patient-identifier="123" patient-ssin="00000000000" patient-full-name="Patient Full Name" view="caregiver" security-label="https://www.apps.vitalink-services.be|PATIENT_HEALTH_RECORD"></vitalink-vaccination-table>
```
...
- Improve display of mapped FHIR resources

### 0.0.10

- Improve mapping of FHIR resource data

### 0.0.9

- Display organization and practitioner name
